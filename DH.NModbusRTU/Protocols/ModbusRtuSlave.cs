...
/// <summary>Modbus RTU 串口从机（Slave/Server）</summary>
/// <remarks>
/// 监听串口，响应 Master 的 FC01/02/03/04/05/06/15/16 请求。
/// 使用后台线程轮询串口数据，检测到完整 RTU 帧后处理并回复。
/// </remarks>
public class ModbusRtuSlave : DisposeBase
{
    #region 属性
    ...
    /// <summary>字节超时（帧间隔检测），默认 20ms</summary>
    ...
    public void Start()
    {
        ...
        _running = true;
        _thread = new Thread(ListenLoop) { IsBackground = true, Name = $"ModbusRtuSlave-{PortName}" };
        _thread.Start();

        WriteLog("ModbusRtuSlave.Start {0} Baudrate={1}", PortName, Baudrate);
    }
    ...
    private void ListenLoop()
    {
        ...
        while (_running)
        {
            try
            {
                ...
                Array.Copy(buf, frame, count);
                var response = ProcessRequest(frame);
            ...
            {
                WriteLog("ModbusRtuSlave.ListenLoop 异常: {0}", ex.Message);
                if (!_running) break;
                Thread.Sleep(100);
            }
        }
    ...
    /// <summary>处理一帧 RTU 请求，返回 RTU 响应字节；帧非法则返回 null</summary>
    /// <param name="frame">完整 RTU 帧（含 CRC）</param>
    /// <returns>RTU 响应帧（含 CRC），或 null（校验失败/不属于本机）</returns>
    public Byte[]? ProcessRequest(Byte[] frame)
    {
        if (frame == null || frame.Length < 4) return null;

        // 验证 CRC（最后 2 字节）
        var crc = ModbusHelper.Crc(frame, 0, frame.Length - 2);
        var crcInFrame = (UInt16)(frame[frame.Length - 2] | (frame[frame.Length - 1] << 8));
        if (crc != crcInFrame)
        {
            WriteLog("CRC 校验失败: 计算={0:X4}, 帧内={1:X4}", crc, crcInFrame);
            return null;
        }

        // 站号过滤
        var frameHost = frame[0];
        var isBroadcast = frameHost == 0;
        if (!isBroadcast && Host != 0 && frameHost != Host) return null;

        // 解析功能码
        var code = (FunctionCodes)frame[1];
        var payloadLen = frame.Length - 4;
        var payload = new Byte[payloadLen < 0 ? 0 : payloadLen];
        if (payloadLen > 0) Array.Copy(frame, 2, payload, 0, payloadLen);

        // 构造响应 RTU 消息（使用 ModbusRtuMessage）
        var rqMsg = new ModbusRtuMessage { Host = frameHost, Code = code };
        rqMsg.Payload = (ArrayPacket)payload;
        ...
        switch (code)
        {
            ...
            case FunctionCodes.ReadDiscrete:
                if (!isBroadcast)
                {
                    var (addr0, cnt0) = rqMsg.GetRequest();
                    BeforeRead?.Invoke(code, addr0, cnt0);
                    rsPayload = OnReadCoil(rqMsg);
                }
                else
                    rsPayload = null;
                break;
            ...
            case FunctionCodes.ReadInput:
                if (!isBroadcast)
                {
                    var (addr1, cnt1) = rqMsg.GetRequest();
                    BeforeRead?.Invoke(code, addr1, cnt1);
                    rsPayload = OnReadRegister(rqMsg);
                }
                else
                    rsPayload = null;
                break;
            ...
            case FunctionCodes.WriteCoil:
                rsPayload = OnWriteCoil(rqMsg);
                break;

            case FunctionCodes.WriteRegister:
                rsPayload = OnWriteRegister(rqMsg);
                break;

            case FunctionCodes.WriteCoils:
                rsPayload = OnWriteCoils(rqMsg);
                break;

            case FunctionCodes.WriteRegisters:
                rsPayload = OnWriteRegisters(rqMsg);
                break;
            ...
            case FunctionCodes.MaskWriteRegister:
                rsPayload = OnMaskWriteRegister(rqMsg);
                break;
        ...
        // 构造响应帧
        var rsMsg = new ModbusRtuMessage { Host = frameHost, Code = code, Payload = rsPayload };
        var rsPacket = rsMsg.ToPacket();
        return rsPacket.ReadBytes();
    }
    #endregion

    #region 功能码处理（与 ModbusSession 逻辑一致）
    ...
    private IPacket? OnReportServerId()
    {
        ...
        var byteCount = (Byte)2;  // ServerId + RunIndicator
        var rs = new Byte[1 + byteCount];
        rs[0] = byteCount;
        rs[1] = ServerId;
        rs[2] = runIndicator;
